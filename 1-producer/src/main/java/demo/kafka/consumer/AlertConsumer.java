...
public class AlertConsumer {
  ...
  public static void main(String[] args) {
    ...
    kaProperties.put("group.id", "demo_team0groupalert");
    /** Deserialize key using {@link AlertKeySerde} */
  ...
  private void consume(final Properties kaProperties) {
    ...
    TopicPartition partitionZero = new TopicPartition("demo_alert", 0);
    ...
    while (keepConsuming) {
      ...
      for (ConsumerRecord<Alert, String> record : records) {
        log.info("demo_info offset = {}, key = {}",
                 record.offset(),
                 record.key().getStageId());
        commitOffset(record.offset(), record.partition(), "demo_alert", consumer);
      }
    }
  }
  ...
  private static void onComplete(Map<TopicPartition, OffsetAndMetadata> map,
    ...
    if (e != null) {
      for (TopicPartition key : map.keySet()) {
        log.info("demo_error topic {}, partition {}, offset {}",
                 key.topic(),
                 key.partition(),
                 map.get(key).offset());
      }
      ...
      for (TopicPartition key : map.keySet()) {
        log.info("demo_info topic {}, partition {}, offset {}",
                 key.topic(),
                 key.partition(),
                 map.get(key).offset());
      }
